...
class ConfigManager:
    ...
    def save_tool_offsets(self, tool_number: int, offsets: Dict[str, float]) -> None:
        ...
        if os.path.exists(self.config_path):
            with open(self.config_path, "r", encoding="utf-8") as f:
                content = f.read()
            pattern = re.compile(rf"^\[tool\s+(?:t)?{tool_number}\]", re.IGNORECASE | re.MULTILINE)
        ...
        if has_legacy:
            ...
            for axis, val in offsets.items():
                if val is not None:
                    offset_vars[f"t{tool_number}_{axis.lower()}"] = val
            self.save_section("tool_offsets", offset_vars)

    def save_all_tool_offsets(self, results: Dict[int, Dict[str, float]]) -> None:
        """
        Atomically updates all tool offsets into the unified [tool_offsets] section
        as variables (t1_x, t1_y, t1_z, ...), preventing duplicate section header
        crashes in Klipper.
        """
        self.create_backup()
        ...
        for t_num, offsets in sorted(results.items()):
            if t_num in legacy_tools:
                ...
                self.save_section(f"tool {t_num}", legacy_dict)
            else:
                for axis, val in offsets.items():
                    if val is not None:
        ...
        if modern_offsets:
            self.save_section("tool_offsets", modern_offsets)
